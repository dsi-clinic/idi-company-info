...
1. **PermID Retrieval** — resolve identifiers to PermID URLs
   ...
   - Ticker: one Record Match API call per batch of up to 1,000 rows
...
| Type | API strategy | Input column | Default batch size |
|------|-------------|--------------|-------------------|
| `cik` | Entity Search | `investor_name`, `investor_cik` | 2,450 |
| `cusip` | Entity Search | `issuer_name`, `cusip` | 2,450 |
| `ticker` | Record Match | `issuer_name`, `ticker` | 2,450 |
...
| `--match-score-threshold` | No | `1` | Minimum Record Match score (ticker only; `1` = 100%) |
...
- Record Match (ticker): CSV payload is further chunked at 1,000 rows per API request to stay within the API size limit
...
| `INPUT_FILE_CIK` | `./data/input/investors_cik.parquet` | Input for CIK track |
...
| `MATCH_SCORE_THRESHOLD` | `1` | Ticker match score floor |
